...
# Instancia o logger a partir da sua classe personalizada
logger = ColorLogger()
# --- FIM DA CONFIGURAÇÃO ---
...
def get_video_duration(caminho_video):
    ...
    try:
        ...
        elif 'streams' in metadata and len(metadata['streams']) > 0 and 'duration' in metadata['streams'][0]:
            return float(metadata['streams'][0]['duration'])
        else:
            logger.warning("Não foi possível encontrar a informação de duração no metadata do vídeo.")
            return None
...
def cortar_video(caminho_entrada, caminho_saida, inicio_corte_segundos=120, duracao_corte_segundos=120):
    """
    ...
    if duracao_total < 300:
        logger.warning(f"Vídeo ignorado. Duração ({int(duracao_total)}s) é menor que 5 minutos.")
        return 'IGNORADO'
    
    logger.info(f"Duração total: {int(duracao_total)}s. O vídeo é elegível para o corte.")
    logger.info(f"Iniciando o corte a partir de {inicio_corte_segundos}s com duração de {duracao_corte_segundos}s.")

    # Comando FFmpeg com o novo ponto de início (-ss)
    # -ss: seek (pular para) o tempo especificado
    comando_ffmpeg = [
        'ffmpeg',
        '-ss', str(inicio_corte_segundos), # Ponto de início do corte
        '-i', caminho_entrada,
        '-t', str(duracao_corte_segundos), # Duração do corte
        '-c:v', 'libx264',
        '-preset', 'veryfast',
        '-crf', '23',
        '-c:a', 'aac',
    ...
    try:
        ...
        logger.error("ERRO CRÍTICO: O comando 'ffmpeg' ou 'ffprobe' não foi encontrado.")
        return 'ERRO'
