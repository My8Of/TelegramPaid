...
class ColorLogger:
    def __init__(self, name='minha_app'):
        ...
        self.log.setLevel(logging.INFO) # Define o nível de log mais baixo
...
class VideoHashManager:
    ...
    def __init__(self, pasta_videos, pasta_banco='banco_dados', nome_csv='mapeamento_titulos.csv'):
        """
    ...
    def carregar_csv_existente(self):
        ...
        if os.path.exists(self.arquivo_csv):
            try:
                ...
                self.logger.info(f"CSV existente carregado: {len(dados)} registros encontrados.")
    ...
    def salvar_no_csv(self, dados):
        ...
        try:
            with open(self.arquivo_csv, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                writer.writerow(['titulo_original', 'hash_md5', 'nome_arquivo_original', 'nome_arquivo_hash'])
                for titulo, info in dados.items():
                    writer.writerow([titulo, info['hash_md5'], info['nome_arquivo_original'], info['nome_arquivo_hash']])
    ...
    def processar_e_renomear_videos(self):
        ...
        arquivos = [f for f in os.listdir(self.pasta_videos) 
                   if os.path.isfile(os.path.join(self.pasta_videos, f)) and 
                   any(f.lower().endswith(ext) for ext in extensoes_video)]
        
        if not arquivos:
            self.logger.warning(f"Nenhum arquivo de vídeo encontrado na pasta '{self.pasta_videos}'.")
            return dados_existentes
        
        self.logger.info(f"Encontrados {len(arquivos)} arquivos de vídeo para processar.")
        ...
        for i, nome_arquivo in enumerate(arquivos, 1):
            ...
            if nome_base in dados_existentes:
                self.logger.info(f"[{i}/{len(arquivos)}] '{nome_arquivo}' já existe no banco de dados.")
                continue
            ...
            self.logger.info(f"[{i}/{len(arquivos)}] Processado: '{nome_arquivo}' -> '{nome_hash}'")
        ...
        if dados_novos:
            ...
            self.logger.info(f"Processamento concluído: {len(dados_novos)} novos registros adicionados.")
            ...
            for titulo_base, info in dados_novos.items():
                ...
                if os.path.exists(caminho_original) and not os.path.exists(caminho_hash):
                    try:
                        os.rename(caminho_original, caminho_hash)
                        self.logger.info(f"Renomeado: '{nome_original}' -> '{nome_hash}'")
            ...
            self.logger.info(f"Renomeação concluída: {arquivos_renomeados} arquivos renomeados.")
        else:
