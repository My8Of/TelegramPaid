...
class DriveManeger:
    def __init__(self):
        base_dir = os.path.dirname(os.path.abspath(__file__)) # Garantindo que use o caminho absoluto
        # Se modificar os scopes, delete o arquivo token.json.
        self.scopes = ['https://www.googleapis.com/auth/drive.readonly']
        self.folder_id = '1_Rvzryv1nMUImQc0JQpqRtpVm1jMfle5'
        self.client_secrets_file = os.path.join(base_dir,'oauth/client_secret_477730350957-vsrp76iaj876gan3psbrll2r0cr3130u.apps.googleusercontent.com.json')
    ...
    def _download_video_from_drive(self,service,file_id, file_name, file_size):
        ...
        try:
            ...
            logger.warning(f"Aviso: Não foi possível obter o tamanho exato do arquivo '{file_name}'. A barra de progresso pode não ser precisa.")
            file_size_int = 0 # Define como 0 para ter uma barra de progresso genérica

        fh = io.FileIO(file_name, 'wb')
        downloader = MediaIoBaseDownload(fh, request, chunksize=1024*1024) # Ajusta o tamanho do chunk para a barra de progresso

        done = False
        downloaded_bytes = 0

        # Inicializa a barra de progresso
        with tqdm(total=file_size_int, unit='B', unit_scale=True, desc=file_name, ncols=80) as pbar:
            while done is False:
