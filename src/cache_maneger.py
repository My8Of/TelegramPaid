...
class CacheManeger:
    ...
    """
    def __init__(self, host='redis-13315.c308.sa-east-1-1.ec2.redns.redis-cloud.com', port=13315, db=0):
        """
        ...
        if not all([os.getenv("REDIS_USER"), os.getenv("REDIS_PASSWORD")]):
            raise ValueError("REDIS_USER e REDIS_PASSWORD não encontrados no arquivo .env")
        self.host = host
        self.port = port
        self.db = db
        self.conn = None
...
if __name__ == '__main__':
    ...
    # 1. Cria uma instância da classe, conectando ao Redis local
    db_manager = CacheManeger(db=1)

    # Verifica se a conexão foi bem-sucedida
    if db_manager.is_connected():

        # 2. Salva um par de chave-valor
        db_manager.set_data("minha_chave", "Olá, Redis!")

        # 3. Recupera o valor
        valor_recuperado = db_manager.get_data("minha_chave")

        # 4. Tenta buscar uma chave que não existe
        db_manager.get_data("chave_inexistente")
