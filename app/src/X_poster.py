...
# Mantenha os imports que já temos
import backoff
from src.utils import ColorLogger
# ...
...
def postar_video_no_twitter(API_KEY, API_KEY_SECRET, ACCESS_TOKEN, ACCESS_TOKEN_SECRET, caminho_do_video, texto_do_tweet):
    """
    ...
    if not all([API_KEY, API_KEY_SECRET, ACCESS_TOKEN, ACCESS_TOKEN_SECRET]):
        logger.error("Uma ou mais chaves da API do Twitter não foram encontradas no arquivo .env.")
        return False
    ...
    media = api_v1.media_upload(filename=caminho_do_video, chunked=True, media_category='tweet_video')
    ...
    if media.processing_info['state'] == 'failed':
        logger.error(f"O processamento do vídeo pelo Twitter falhou: {media.processing_info['error']}")
        return False
